...
<style>
    ...
    .detail-badge i { color: var(--accent); }
    ...
    .spec-item .value { font-weight: 600; color: var(--primary); }
    ...
    .book-btn:hover { background: var(--accent-light); transform: translateY(-1px); box-shadow: 0 4px 16px rgba(184,150,46,0.4); }
    ...
    .modal-booking .modal-header .btn-close { filter: invert(1); }
...
<div class="container py-4">
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            ...
            @else
            <img src="{{ asset('images/no-image.svg') }}" class="w-100 rounded-3 mb-3" alt="{{ $property->title }}" style="height:340px;object-fit:cover;">
            @endif
            ...
            <div class="property-detail-box">
                <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mb-2">
                    <div>
                        <span class="badge me-1" style="background:{{ $property->listing_type === 'rent' ? 'var(--accent)' : 'var(--primary)' }};font-size:0.72rem;">
                            {{ $property->listing_type === 'rent' ? __('messages.common.for_rent') : __('messages.common.for_sale') }}
                        </span>
                        <span class="badge me-1" style="background:rgba(26,43,74,0.1);color:var(--primary);font-size:0.72rem;">{{ __('messages.types.'.$property->type) }}</span>
                        @if($property->category)
                        <span class="badge" style="background:rgba(184,150,46,0.15);color:var(--accent);font-size:0.72rem;">{{ $property->category->name }}</span>
                        @endif
                    </div>
                    <small class="text-muted" style="font-size:0.78rem;"><i class="bi bi-eye me-1"></i>{{ number_format($property->views_count) }} {{ __('messages.property.views') }}</small>
                </div>
                <h1 style="font-family:'Cormorant Garamond',serif;font-size:1.9rem;color:var(--primary);line-height:1.3;">{{ $property->title }}</h1>
                <p class="text-muted mb-3" style="font-size:0.9rem;"><i class="bi bi-geo-alt-fill me-1" style="color:var(--accent);"></i>{{ $property->location_text }}</p>

                <!-- Quick specs badges -->
                <div>
                    <span class="detail-badge"><i class="bi bi-rulers"></i>{{ $property->size }} {{ $property->size_unit }}</span>
                    @if($property->bedrooms)
                    <span class="detail-badge"><i class="bi bi-door-closed"></i>{{ $property->bedrooms }} {{ __('messages.property.bedrooms') }}</span>
                    @endif
                    @if($property->bathrooms)
                    <span class="detail-badge"><i class="bi bi-droplet"></i>{{ $property->bathrooms }} {{ __('messages.property.bathrooms') }}</span>
                    @endif
                    <span class="detail-badge"><i class="bi bi-check-circle"></i>{{ ucfirst($property->status) }}</span>
                </div>
            </div>

            <!-- Description -->
            <div class="property-detail-box">
                <h4 style="font-family:'Cormorant Garamond',serif;color:var(--primary);margin-bottom:1rem;">{{ __('messages.property.description') }}</h4>
                <div style="font-size:0.92rem;line-height:1.85;color:var(--text);">{!! nl2br(e($property->description)) !!}</div>
            </div>

            <!-- Specifications Table -->
            <div class="property-detail-box">
                <h4 style="font-family:'Cormorant Garamond',serif;color:var(--primary);margin-bottom:1rem;">{{ __('messages.property.details') }}</h4>
                <div class="row">
                    <div class="col-md-6">
                        <div class="spec-item">
                            <span class="label">{{ __('messages.property.id') }}</span>
                            <span class="value">#{{ str_pad($property->id, 5, '0', STR_PAD_LEFT) }}</span>
                        </div>
                        <div class="spec-item">
                        ...
                        <div class="spec-item">
                            ...
                            <span class="value">{{ $property->listing_type === 'rent' ? __('messages.common.for_rent') : __('messages.common.for_sale') }}</span>
                        </div>
                        <div class="spec-item">
            ...
            <div class="property-detail-box">
                <h4 style="font-family:'Cormorant Garamond',serif;color:var(--primary);margin-bottom:1rem;">
                    <i class="bi bi-play-circle me-2" style="color:var(--accent);"></i>{{ __('messages.property.video_tour') }}
                </h4>
            ...
            <div class="property-detail-box">
                <h4 style="font-family:'Cormorant Garamond',serif;color:var(--primary);margin-bottom:1rem;">
                    <i class="bi bi-facebook me-2" style="color:#1877f2;"></i>{{ __('messages.property.fb_video_tour') }}
                </h4>
                <div style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;border-radius:var(--radius);">
                    <iframe
                        src="{{ $property->facebook_embed_url }}"
                ...
                <p style="font-size:0.78rem;color:var(--text-muted);margin-top:0.5rem;">
                    <i class="bi bi-info-circle me-1"></i>{!! __('messages.property.fb_video_hint', ['public' => '<strong>'.__('messages.property.public').'</strong>']) !!}
                </p>
            </div>
            @endif
            ...
            <div class="property-detail-box">
                <h4 style="font-family:'Cormorant Garamond',serif;color:var(--primary);margin-bottom:1rem;">
                    <i class="bi bi-geo-alt me-2" style="color:var(--accent);"></i>{{ __('messages.property.location_map') }}
                </h4>
        ...
        <div class="col-lg-4">
            <!-- Price box -->
            <div class="price-box">
                <div class="mb-1" style="font-size:0.8rem;color:rgba(255,255,255,0.6);letter-spacing:1px;text-transform:uppercase;">{{ __('messages.property.listed_price') }}</div>
                <div class="price-main">{!! $property->price_html !!}</div>
                @if($property->price_unit)
                <div style="color:rgba(255,255,255,0.65);font-size:0.82rem;margin-top:0.3rem;">{{ $property->price_unit }}</div>
                @endif
                ...
                @else
                <div class="mt-3 p-2 text-center rounded" style="background:rgba(255,255,255,0.1);color:rgba(255,255,255,0.8);font-size:0.88rem;">
            ...
            <!-- Contact card -->
            <div class="property-detail-box">
                <h6 style="font-family:'Cormorant Garamond',serif;color:var(--primary);font-size:1.1rem;margin-bottom:1rem;">{{ __('messages.property.need_help') }}</h6>
                <p style="font-size:0.85rem;color:var(--text-muted);margin-bottom:1rem;">{{ __('messages.property.need_help_text') }}</p>
                <a href="tel:{{ preg_replace('/\s+/', '', $siteSettings['company_phone'] ?? '+8801700000000') }}" class="btn btn-outline-accent w-100 rounded-pill mb-2">
                    <i class="bi bi-telephone me-2"></i>{{ $siteSettings['company_phone'] ?? '+880 1700-000000' }}
                </a>
                <a href="{{ route('contact') }}" class="btn btn-outline-secondary w-100 rounded-pill" style="font-size:0.88rem;">
            ...
            <!-- Share -->
            <div class="property-detail-box">
                <h6 style="font-family:'Cormorant Garamond',serif;color:var(--primary);font-size:1rem;margin-bottom:1rem;">{{ __('messages.property.share') }}</h6>
                <div class="d-flex gap-2">
                    <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}" target="_blank"
                        class="btn btn-sm rounded-pill px-3" style="background:#1877f2;color:#fff;font-size:0.8rem;">
                        <i class="bi bi-facebook me-1"></i>Facebook
                    </a>
                    <a href="https://wa.me/?text={{ urlencode($property->title . ' ' . url()->current()) }}" target="_blank"
                        class="btn btn-sm rounded-pill px-3" style="background:#25d366;color:#fff;font-size:0.8rem;">
    ...
    @if($related->count())
    <div class="mt-5">
        <h3 style="font-family:'Cormorant Garamond',serif;color:var(--primary);margin-bottom:1.5rem;">{{ __('messages.property.similar') }}</h3>
        <div class="row g-4">
        ...
        <div class="modal-content border-0">
            ...
            <div class="modal-body p-4">
                ...
                <form id="bookingForm" method="POST" action="{{ route('bookings.store', $property->slug) }}">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold" style="font-size:0.88rem;">{{ __('messages.booking.phone') }} <span class="text-danger">*</span></label>
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold" style="font-size:0.88rem;">{{ __('messages.booking.email') }}</label>
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold" style="font-size:0.88rem;">{{ __('messages.booking.message') }}</label>
                        <textarea name="message"
                                  class="form-control @error('message') is-invalid @enderror"
                                  rows="3" placeholder="{{ __('messages.booking.message_ph') }}">{{ old('message') }}</textarea>
...
$(function() {
    ...
    if (document.querySelector('.galleryMain')) {
        ...
        if (galleryThumbs) mainOpts.thumbs = { swiper: galleryThumbs };
