    ...
    <div class="container">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-2" style="font-size:0.8rem;">
                <li class="breadcrumb-item"><a href="{{ route('home') }}" style="color:rgba(255,255,255,0.6);">{{ __('messages.nav.home') }}</a></li>
                <li class="breadcrumb-item active" style="color:rgba(255,255,255,0.9);">{{ __('messages.nav.properties') }}</li>
            </ol>
        </nav>
        <h1 style="font-family:'Cormorant Garamond',serif;color:#fff;font-size:2.4rem;margin-bottom:0.3rem;">{{ __('messages.property.all_properties') }}</h1>
        ...
        <div class="col-lg-3">
            <div class="filter-sidebar" id="filterSidebar">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h5 style="font-family:'Cormorant Garamond',serif;color:var(--primary);font-size:1.3rem;margin:0;">{{ __('messages.filter.title') }}</h5>
                    <a href="{{ route('properties.index') }}" class="btn btn-sm btn-outline-secondary rounded-pill" style="font-size:0.75rem;">{{ __('messages.filter.clear_all') }}</a>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="d-flex gap-2">
                        <button class="btn btn-sm filter-type-btn {{ ($filters['listing_type'] ?? '') === '' ? 'btn-primary-custom active' : 'btn-outline-secondary' }} rounded-pill" data-filter="listing_type" data-val="">{{ __('messages.common.all') }}</button>
                        <button class="btn btn-sm filter-type-btn {{ ($filters['listing_type'] ?? '') === 'sale' ? 'btn-primary-custom active' : 'btn-outline-secondary' }} rounded-pill" data-filter="listing_type" data-val="sale">{{ __('messages.common.sale') }}</button>
                        <button class="btn btn-sm filter-type-btn {{ ($filters['listing_type'] ?? '') === 'rent' ? 'btn-primary-custom active' : 'btn-outline-secondary' }} rounded-pill" data-filter="listing_type" data-val="rent">{{ __('messages.common.rent') }}</button>
                    </div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="form-check">
                        <input class="form-check-input filter-check" type="radio" name="type_radio" id="type_{{ $val }}"
                            data-filter="type" data-val="{{ $val }}" {{ ($filters['type'] ?? '') === $val ? 'checked' : '' }}>
                        <label class="form-check-label" for="type_{{ $val }}">{{ __('messages.types.'.$val) }}</label>
                    </div>
                    @endforeach
                </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="d-flex price-range-inputs">
                        <input type="number" class="form-control form-control-sm filter-input" data-filter="min_price" placeholder="{{ __('messages.filter.min') }}" value="{{ $filters['min_price'] ?? '' }}">
                        <input type="number" class="form-control form-control-sm filter-input" data-filter="max_price" placeholder="{{ __('messages.filter.max') }}" value="{{ $filters['max_price'] ?? '' }}">
                    </div>
                </div>
...
$(function() {
    ...
    $(document).on('click', '.filter-type-btn', function() {
        $('.filter-type-btn[data-filter="' + $(this).data('filter') + '"]').removeClass('active btn-primary-custom').addClass('btn-outline-secondary');
