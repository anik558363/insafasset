    ...
    <div class="col-lg-8">
            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <input type="text" name="title" class="form-control"
                        ...
                        placeholder="e.g. Premium Land in Bashundhara R/A" required>
                </div>
                <div class="mb-3">
                    <label class="form-label">Description <span class="text-danger">*</span></label>
                    <textarea name="description" class="form-control" rows="6" placeholder="Detailed property description..."
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="category_id" class="form-select" required>
                            ...
                            @foreach ($categories as $cat)
                                <option value="{{ $cat->id }}"
                                    {{ old('category_id', $property->category_id ?? '') == $cat->id ? 'selected' : '' }}>
                                    {{ $cat->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        ...
                        <select name="type" class="form-select" required id="typeSelect">
                            ...
                            @foreach (['land' => 'Land', 'flat' => 'Flat', 'house' => 'House', 'commercial' => 'Commercial'] as $v => $l)
                                <option value="{{ $v }}"
                                    {{ old('type', $property->type ?? '') === $v ? 'selected' : '' }}>
                                    {{ $l }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        ...
                        <select name="listing_type" class="form-select" required>
                            @foreach (['sale' => 'For Sale', 'rent' => 'For Rent'] as $v => $l)
                                <option value="{{ $v }}"
                                    {{ old('listing_type', $property->listing_type ?? '') === $v ? 'selected' : '' }}>
                                    {{ $l }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="row g-3">
                    <div class="col-md-5">
                        ...
                        <input type="text" name="price" class="form-control"
                            value="{{ old('price', $property->price ?? '') }}" placeholder=" Enter Price " required>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Price Unit</label>
                        <input type="text" name="price_unit" class="form-control"
                            value="{{ old('price_unit', $property->price_unit ?? '') }}"
                            placeholder="e.g. per katha, total">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Size <span class="text-danger">*</span></label>
                        <input type="text" name="size" class="form-control"
                            value="{{ old('size', $property->size ?? '') }}" placeholder=" Enter size " required>
                    </div>
                    <div class="col-md-3">
                        ...
                        <select name="size_unit" class="form-select" required>
                            @foreach (['katha', 'bigha', 'decimal', 'sft', 'acre'] as $u)
                                <option value="{{ $u }}"
                                    {{ old('size_unit', $property->size_unit ?? 'katha') === $u ? 'selected' : '' }}>
                                    {{ strtoupper($u) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3 bedroom-bath-field">
                        ...
                        <input type="number" name="bedrooms" class="form-control"
                            value="{{ old('bedrooms', $property->bedrooms ?? '') }}" placeholder="0" min="0">
                    </div>
                    <div class="col-md-3 bedroom-bath-field">
                        <label class="form-label">Bathrooms</label>
                        <input type="number" name="bathrooms" class="form-control"
                            value="{{ old('bathrooms', $property->bathrooms ?? '') }}" placeholder="0" min="0">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <input type="text" name="location_text" class="form-control"
                        value="{{ old('location_text', $property->location_text ?? '') }}"
                        placeholder="e.g. Block-C, Bashundhara R/A, Dhaka" required>
                </div>
                <div class="row g-3">
                    <div class="col-md-4">
                        <label class="form-label">Division</label>
                        <input type="text" name="division" class="form-control"
                            value="{{ old('division', $property->division ?? '') }}" placeholder="e.g. Dhaka">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">District</label>
                        <input type="text" name="district" class="form-control"
                            value="{{ old('district', $property->district ?? '') }}" placeholder="e.g. Dhaka">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Area / Thana</label>
                        <input type="text" name="area" class="form-control"
                            value="{{ old('area', $property->area ?? '') }}" placeholder="e.g. Gulshan">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Latitude</label>
                        <input type="text" name="latitude" class="form-control"
                            value="{{ old('latitude', $property->latitude ?? '') }}" placeholder="23.7465">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Longitude</label>
                        <input type="text" name="longitude" class="form-control"
                            value="{{ old('longitude', $property->longitude ?? '') }}" placeholder="90.3760">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="admin-card mb-4">
            ...
            <div class="card-body">
                ...
                @if (isset($property) && $property->images->count())
                    ...
                    <div class="img-preview-grid mb-3" id="existingImages">
                        @foreach ($property->images->sortBy('sort_order') as $img)
                            <div class="img-preview-item" data-id="{{ $img->id }}"
                                data-order="{{ $img->sort_order }}">
                                <img src="{{ $img->url }}" alt="">
                                @if ($img->is_primary)
                                    <div
                                        style="position:absolute;bottom:2px;left:2px;background:var(--accent);color:#fff;font-size:0.55rem;padding:1px 5px;border-radius:3px;">
                                        PRIMARY</div>
                                @endif
                                <button type="button" class="remove-img" data-id="{{ $img->id }}"
                    ...
                    <p style="font-size:0.78rem;color:var(--text-muted);margin-bottom:1rem;"><i
                            class="bi bi-info-circle me-1"></i>Drag to reorder. Click × to mark for deletion. Changes
                        save on form submit.</p>
                @endif
                ...
                <div style="font-size:0.78rem;color:var(--text-muted);margin-top:0.5rem;">Max 5MB per image. JPG, PNG,
                    WEBP. First image becomes primary.</div>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <input type="text" name="meta_title" class="form-control"
                        value="{{ old('meta_title', $property->meta_title ?? '') }}" maxlength="200">
                </div>
                <div>
                    <label class="form-label">Meta Description <small class="text-muted">(max 300
                            chars)</small></label>
                ...
                <div class="mb-3">
                    ...
                    <select name="status" class="form-select" required>
                        @foreach (['available' => 'Available', 'booked' => 'Booked', 'sold' => 'Sold', 'rented' => 'Rented'] as $v => $l)
                            <option value="{{ $v }}"
                                {{ old('status', $property->status ?? 'available') === $v ? 'selected' : '' }}>
                                {{ $l }}</option>
                        @endforeach
                    </select>
                </div>
        ...
        <div class="admin-card mb-4">
            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <div style="font-size:0.78rem;color:var(--text-muted);margin-top:0.4rem;">Paste any YouTube link —
                        auto-converted to embed format.</div>
                    @if (isset($property) && $property->youtube_embed_url)
                        <div class="ratio ratio-16x9 mt-2">
                            <iframe src="{{ $property->youtube_embed_url }}" title="YouTube Preview"
                ...
                <div>
                    ...
                    <div style="font-size:0.78rem;color:var(--text-muted);margin-top:0.4rem;">
                        <i class="bi bi-info-circle me-1"></i>Supports facebook.com/videos/ and fb.watch links. The
                        video must be public.
                    </div>
                    @if (isset($property) && $property->facebook_embed_url)
                        <div class="ratio ratio-16x9 mt-2" style="max-height:280px;">
                            <iframe src="{{ $property->facebook_embed_url }}" title="Facebook Video Preview"
                    ...
                    reader.onload = (function(idx) {
                        return function(e) {
                            const item = $('<div class="img-preview-item">').append(
                                ...
                                $('<button type="button" class="remove-img">').attr(
                                    'data-new-idx', idx).html('<i class="bi bi-x"></i>')
                            );
