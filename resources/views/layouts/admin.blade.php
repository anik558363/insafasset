...
<head>
    ...
    <style>
        ...
        .sidebar-link .badge { font-size: 0.68rem; margin-left: auto; }
        ...
        .admin-table td { padding: 0.75rem 1rem; vertical-align: middle; border-bottom: 1px solid var(--border); }
        .admin-table tbody tr:hover { background: rgba(26,43,74,0.02); }
        ...
        /* Alerts */
        .alert { font-size: 0.875rem; border-radius: 8px; }

        /* Page title */
        .page-title { font-family: 'Cormorant Garamond', serif; font-size: 1.8rem; color: var(--primary); margin: 0; }
...
<aside class="admin-sidebar" id="adminSidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ route('admin.properties.create') }}" class="sidebar-link {{ request()->routeIs('admin.properties.create') ? 'active' : '' }}">
        ...
        <a href="{{ route('admin.bookings.index') }}" class="sidebar-link {{ request()->routeIs('admin.bookings.*') ? 'active' : '' }}">
            ...
            @php try { $pendingCount = \App\Models\Booking::where('status','pending')->count(); } catch(\Throwable $e) { $pendingCount = 0; } @endphp
            @if($pendingCount > 0)
        ...
        <a href="{{ route('admin.messages.index') }}" class="sidebar-link {{ request()->routeIs('admin.messages.*') ? 'active' : '' }}">
            ...
            @php try { $unreadCount = \App\Models\ContactMessage::where('is_read',false)->count(); } catch(\Throwable $e) { $unreadCount = 0; } @endphp
            @if($unreadCount > 0)
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="sidebar-link w-100 text-start border-0 bg-transparent" style="color:rgba(255,255,255,0.7);">
...
<header class="admin-topbar">
    ...
    <div class="d-flex align-items-center gap-3">
        <div style="font-size:0.85rem;color:var(--text-muted);">
            ...
            <span class="badge ms-1" style="background:var(--accent);font-size:0.65rem;">{{ ucfirst(auth()->user()->role) }}</span>
        </div>
    </div>
</header>
