...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}"
    dir="{{ config('app.supported_locales.' . app()->getLocale() . '.dir', 'ltr') }}">
...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@400;500;600;700&family=Hind+Siliguri:wght@400;500;600;700&family=Inter:wght@300;400;500;600;700&display=swap"
        rel="stylesheet">
    ...
    <style>
        ...
        * {
        ...
        html[lang="bn"] .card-price,
        html[lang="bn"] .stat-number,
        html[lang="bn"] .price-main {
            font-family: 'Hind Siliguri', 'Inter', sans-serif;
        }
                ...
                <ul class="navbar-nav ms-auto align-items-lg-center gap-lg-1">
                    ...
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('properties.*') ? 'active' : '' }}"
                            href="{{ route('properties.index') }}">{{ __('messages.nav.properties') }}</a>
                    </li>
                    <li class="nav-item">
                        ...
                        @if (auth()->user()->isAdmin() || auth()->user()->isAgent())
                            <li class="nav-item">
                                <a class="nav-link" href="{{ route('admin.dashboard') }}"><i
                                        class="bi bi-speedometer2 me-1"></i>{{ __('messages.nav.admin') }}</a>
                            </li>
                        @endif
                                ...
                                <li>
                                    <form method="POST" action="{{ route('logout') }}">
                                        ...
                                        <button type="submit" class="dropdown-item"><i
                                                class="bi bi-box-arrow-right me-2"></i>{{ __('messages.nav.logout') }}</button>
                                    </form>
                                </li>
                            </ul>
                        </li>
                    @else
                        <li class="nav-item ms-lg-2">
                            <a href="{{ route('login') }}"
                                class="btn btn-outline-light btn-sm rounded-pill px-3">{{ __('messages.nav.login') }}</a>
                        </li>
                    @endauth
    ...
    <footer class="footer-main mt-5">
        <div class="container">
            <div class="row g-4">
                <div class="col-lg-4">
                    @if (!empty($siteSettings['site_logo']))
                        <img src="{{ asset($siteSettings['site_logo']) }}"
                            alt="{{ $siteSettings['site_name'] ?? 'LandMark Realty' }}"
                            style="height:44px;object-fit:contain;margin-bottom:0.75rem;">
                    @else
                        <h5 style="font-size:1.6rem;">{{ $siteSettings['site_name'] ?? 'LandMark' }} <span
                                style="color:#fff;font-weight:400;"> Realty</span></h5>
                    @endif
                    <p style="font-size:0.88rem;line-height:1.8;">
                        {{ $siteSettings['site_tagline'] ?? 'Your most trusted real estate partner in Bangladesh.' }}
                    </p>
                    <div class="footer-social mt-3">
                        ...
                        @if (!empty($siteSettings['company_whatsapp']))
                            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $siteSettings['company_whatsapp']) }}"
                ...
                <div class="col-lg-2 col-sm-6">
                    ...
                    <ul class="list-unstyled">
                        <li class="mb-2"><a
                                href="{{ route('properties.index') }}?listing_type=sale">{{ __('messages.footer.buy_property') }}</a>
                        </li>
                        <li class="mb-2"><a
                                href="{{ route('properties.index') }}?listing_type=rent">{{ __('messages.footer.rent_property') }}</a>
                        </li>
                        <li class="mb-2"><a
                                href="{{ route('properties.index') }}?type=land">{{ __('messages.types.land') }}</a>
                        </li>
                        <li class="mb-2"><a
                                href="{{ route('properties.index') }}?type=flat">{{ __('messages.types.flats') }}</a>
                        </li>
                ...
                <div class="col-lg-2 col-sm-6">
                    ...
                    <ul class="list-unstyled">
                        <li class="mb-2"><a
                                href="{{ route('properties.index') }}?type=house">{{ __('messages.types.houses') }}</a>
                        </li>
                        <li class="mb-2"><a
                                href="{{ route('properties.index') }}?type=commercial">{{ __('messages.types.commercial') }}</a>
                        </li>
                        <li class="mb-2"><a href="{{ route('properties.index') }}?division=Dhaka">Dhaka</a></li>
                        <li class="mb-2"><a
                                href="{{ route('properties.index') }}?division=Chittagong">Chittagong</a></li>
                        <li class="mb-2"><a href="{{ route('contact') }}">{{ __('messages.nav.contact') }}</a>
                        </li>
                    </ul>
                </div>
                <div class="col-lg-4">
                    ...
                    <ul class="list-unstyled" style="font-size:0.88rem;">
                        @if (!empty($siteSettings['company_address']))
                            <li class="mb-2"><i class="bi bi-geo-alt-fill me-2"
                                    style="color:var(--accent-light);"></i>{{ $siteSettings['company_address'] }}</li>
                        @endif
                        @if (!empty($siteSettings['company_phone']))
                            <li class="mb-2"><i class="bi bi-telephone-fill me-2"
                                    style="color:var(--accent-light);"></i><a
                                    href="tel:{{ preg_replace('/\s+/', '', $siteSettings['company_phone']) }}">{{ $siteSettings['company_phone'] }}</a>
                            </li>
                        @endif
                        ...
                        @if (!empty($siteSettings['company_hours']))
                            <li class="mb-2"><i class="bi bi-clock-fill me-2"
                                    style="color:var(--accent-light);"></i>{{ $siteSettings['company_hours'] }}</li>
                        @endif
                    </ul>
                </div>
            </div>
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center"
                style="font-size:0.82rem;color:rgba(255,255,255,0.5);">
                <span>© {{ date('Y') }}
                    {{ $siteSettings['copyright_text'] ?? 'LandMark Realty. All rights reserved.' }}</span>
